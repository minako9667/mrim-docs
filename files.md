...
Если нет, то сначала клиент-отправитель пытается подключиться к клиенту-получателю, и если и это не получается, отправитель запрашивают у сервера прокси через команду `MRIM_CS_PROXY`. См. [Протокольный прокси](proxy.md)
...
- _0x4_ - от сервера требуется прокси (см. [Протокольный прокси](proxy.md))
